...
#if defined(NDEBUG)
# error "btc4coin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8f0808d92a05c9ff69027d03955e045972ac66f5b81233e6c5f2dcc02aa00170");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // btc4coin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "btc4coin Signed Message:\n";
...
{
    // btc4coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // btc4coin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // btc4coin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // btc4coin: 30 sec.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // btc4coin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // btc4coin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8f0808d92a05c9ff69027d03955e045972ac66f5b81233e6c5f2dcc02aa00170");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "btc4coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("128184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1514563463;
        ...
        assert(block.hashMerkleRoot == uint256("0xd736fb5a17d6140a904f72efc7e3497c724647eafbe358cdaced50739fa84d70"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // btc4coin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("btc4coinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("btc4coinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static btc4coinMiner(CWallet *pwallet)
{
    printf("btc4coinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("btc4coin-miner");
    ...
    try { loop {
        ...
        printf("Running btc4coinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("btc4coinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&btc4coinMiner, pwallet));
}
